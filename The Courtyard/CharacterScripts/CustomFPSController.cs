...
using UnitySampleAssets.Characters.FirstPerson;
	...
	public class CustomFPSController : MonoBehaviour
	{
        ...
        public LadderObject mCurrentLadder;
        public float fLadderSlopeRequirement = 0.1f;
        ...
        {
            if (mCurrentLadder && Mathf.Abs(mCurrentLadder.mLadderCollider.transform.forward.y) > fLadderSlopeRequirement)
                return true;
            else
                return false;
        }
		private void FixedUpdate()
		{
			float speed;
			GetInput(out speed);
        // if on ladder, move towards the end of the ladder the player is moving
        // if not on ladder, just move normally on the plane
            Vector3 desiredMove;
            if (!IsOnLadder())
            {
            desiredMove = mPlayerCharacter.transform.forward * _input.y + mPlayerCharacter.transform.right * _input.x;
            ...
            {
            Vector3 upLadder = mCurrentLadder.mLadderTop.transform.position - mCurrentLadder.mLadderCollider.transform.position;

            if (mCurrentLadder.mLadderTop.transform.forward.y < 0)
                upLadder *= -1;

            desiredMove = upLadder * _input.y;
            desiredMove = Vector3.ProjectOnPlane(desiredMove, mCurrentLadder.mLadderCollider.transform.up).normalized;

            _moveDir.y = desiredMove.y * speed;
			...
			if (_characterController.isGrounded && !IsOnLadder())
			{
